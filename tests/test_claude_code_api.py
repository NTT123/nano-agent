...
MOCK_HEADERS: dict[str, str] = {
    "authorization": "Bearer sk-ant-oat01-test123456789",
...
def mock_load_config() -> Generator[MagicMock, None, None]:
    """Mock load_config to avoid actual file reads."""
...
class TestClaudeCodeAPIInit:
    ...
    def test_init_loads_config(self, mock_load_config: MagicMock) -> None:
        ...
        assert api.auth_token == "sk-ant-oat01-test123456789"

    def test_init_uses_captured_values(self, mock_load_config: MagicMock) -> None:
    ...
    def test_init_with_env_var(self) -> None:
        """Verify CLAUDE_CODE_OAUTH_TOKEN env var is used."""
        with patch("nano_agent.capture_claude_code_auth.load_config") as mock:
            mock.return_value = None  # No config file
            with patch.dict(os.environ, {"CLAUDE_CODE_OAUTH_TOKEN": "sk-ant-oat01-envtoken"}):
                api = ClaudeCodeAPI()
                assert api.auth_token == "sk-ant-oat01-envtoken"

    def test_init_raises_without_auth(self) -> None:
        """Verify clear error if no auth source available."""
        with patch("nano_agent.capture_claude_code_auth.load_config") as mock:
            mock.return_value = None  # No config file
...
class TestClaudeCodeAPISend:
    ...
    async def test_send_uses_captured_headers(
        ...
        with patch.object(api._client, "post", new_callable=AsyncMock) as mock_post:
            ...
            assert headers["authorization"] == "Bearer sk-ant-oat01-test123456789"
